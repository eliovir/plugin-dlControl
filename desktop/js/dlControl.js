...
$(function() {
    $("#table_cmd tbody").delegate(".listCmddlControl", 'click', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
        $('.description.' + $('#sel_addPreConfigCmddlControl').value()).show();
        $('.version.' + $('#sel_addPreConfigCmddlControl').value()).show();
        $('.required.' + $('#sel_addPreConfigCmddlControl').value()).show();
        $('#md_addPreConfigCmddlControl').modal('show');
        $('#bt_addPreConfigCmddlControlSave').undelegate().unbind();
        var tr = $(this).closest('tr');
        $("#div_mainContainer").delegate("#bt_addPreConfigCmddlControlSave", 'click', function(event) {
            var cmd_data = json_decode($('.json_cmd.' + $('#sel_addPreConfigCmddlControl').value()).value());
            tr.setValues(cmd_data, '.cmdAttr');
            $('#md_addPreConfigCmddlControl').modal('hide');
        });
    });

    $("#sel_addPreConfigCmddlControl").on('change', function(event) {
        $('.description').hide();
        $('.version').hide();
        $('.required').hide();
...
function addCmdToTable(_cmd) {
	...
	tr += '<a class="btn btn-default listCmddlControl form-control input-sm" style="margin-top : 5px;"><i class="fa fa-list-alt cursor"></i> {{Ajouter une commande prédéfinie}}</a>';
    tr += '</td>';
    tr += '<td>';
